...
import type { Supplier, PurchaseOrder } from '@/types/purchases';
...
export default function SupplierDetailsPage() {
    ...
    const [orders, setOrders] = useState<Partial<PurchaseOrder>[]>([]);
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const ordersRes = await fetch(`/api/purchases/orders?supplier_id=${supplierId}`);
    ...
    if (error || !supplier) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
                ...
                <p className="text-muted-foreground">{error || 'المورد غير موجود'}</p>
                <Link href="/dashboard/purchases/suppliers" className="mt-4 text-primary hover:underline">
                    العودة للموردين
                </Link>
            </div>
        );
    ...
    const supplierGroup = supplier.supplier_group || 'local';
    const groupInfo = groupConfig[supplierGroup] || groupConfig.local;
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/purchases/suppliers" className="p-2 hover:bg-muted rounded-lg text-muted-foreground">
                        <ArrowRight size={20} />
                    </Link>
                    <div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-card rounded-xl border border-border p-4">
                            ...
                            <div className="text-xl font-bold">{supplier.total_orders || 0}</div>
                        </div>
                        <div className="bg-card rounded-xl border border-border p-4">
                            <div className="flex items-center gap-2 text-muted-foreground mb-1">
                                <TrendingUp size={14} />
                            ...
                            <div className="text-xl font-bold">{(supplier.total_purchases || 0).toLocaleString('ar-EG')}</div>
                        </div>
                        <div className="bg-card rounded-xl border border-border p-4">
                        ...
                        <div className="bg-card rounded-xl border border-border p-4">
                            ...
                            <div className="text-xl font-bold">{(supplier.credit_limit || 0).toLocaleString('ar-EG')}</div>
                        </div>
                    </div>
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <span className={cn(
                                                        ...
                                                        {order.status === 'sent' ? 'مرسل' :
                                                            order.status === 'received' ? 'تم الاستلام' : 'مكتمل'}
                                                    </span>
                                ...
                                <div className="space-y-4">
                                    {supplier.tax_number && (
                                        <div>
                                            ...
                                            <span className="mr-2 font-mono">{supplier.tax_number}</span>
                                        </div>
                                    )}
                                    {(supplier.payment_terms_days || 0) > 0 && (
                                        <div>
                                            <span className="text-sm text-muted-foreground">شروط الدفع:</span>
                                            <span className="mr-2">{supplier.payment_terms_days} يوم</span>
                                        </div>
                                    )}
                                    {supplier.notes && (
                                        <div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-card rounded-xl border border-border p-4">
                        ...
                        <div className="space-y-2">
                            <Link
                                href={`/dashboard/purchases/orders/new?supplier=${supplier.id}`}
