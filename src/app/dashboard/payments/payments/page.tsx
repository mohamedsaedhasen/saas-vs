...
export default function PaymentsPage() {
    ...
    const filteredPayments = payments.filter((payment) => {
        ...
        const supplierName = payment.supplier_name || payment.supplier?.name || '';
        if (searchTerm && !payment.payment_number?.includes(searchTerm) && !supplierName.includes(searchTerm)) return false;
        return true;
    });

    const totalPayments = payments.reduce((sum, p) => sum + (p.amount || 0), 0);
    const cashTotal = payments.filter(p => p.payment_method === 'cash').reduce((sum, p) => sum + (p.amount || 0), 0);
    const bankTotal = payments.filter(p => p.payment_method === 'bank').reduce((sum, p) => sum + (p.amount || 0), 0);
        ...
        <div className="space-y-6 p-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
                        ...
                        <span>/</span>
                        <span className="text-gray-900">سندات الصرف</span>
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900">سندات الصرف</h1>
                </div>
                <Link
                    href="/dashboard/payments/payments/new"
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition-colors"
                >
                    <Plus size={20} />
                    سند صرف جديد
                </Link>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-red-600">{totalPayments.toLocaleString('ar-EG')} ج.م</div>
                </div>
                <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm flex flex-wrap items-center gap-4">
                <div className="flex-1 min-w-[200px]">
                    <div className="relative">
                        <input
                            type="text"
                            placeholder="بحث برقم السند أو المورد..."
                            value={searchTerm}
            ...
            {filteredPayments.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-white rounded-2xl shadow-sm">
                    ...
                    <h3 className="text-lg font-semibold mb-2">لا توجد سندات صرف</h3>
                    <p className="text-gray-500 mb-4">ابدأ بإنشاء سند صرف جديد</p>
                    <Link
                        href="/dashboard/payments/payments/new"
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-600">المورد</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredPayments.map((payment) => {
                                ...
                                const vaultName = payment.vault_or_bank || payment.vault?.name || '';
                                    ...
                                    <tr key={payment.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-mono font-medium text-gray-900">{payment.payment_number}</div>
                                                    <div className="text-xs text-gray-500">{payment.reference || ''}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-gray-600">
                                            {paymentDate ? new Date(paymentDate).toLocaleDateString('ar-EG') : '-'}
                                        </td>
                                        <td className="px-4 py-3 text-gray-900">{supplierName}</td>
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${methodColors[payment.payment_method] || 'bg-gray-100 text-gray-700'}`}>
                                                {methodLabels[payment.payment_method] || payment.payment_method}
                                            </span>
                                            {vaultName && <div className="text-xs text-gray-500 mt-1">{vaultName}</div>}
                                        </td>
                                        <td className="px-4 py-3 text-left font-mono font-bold text-red-600">
