...
export default function ReturnsPage() {
    ...
    const filteredReturns = returns.filter((ret) => {
        if (searchTerm && !ret.return_number?.includes(searchTerm) && !ret.customer_name?.includes(searchTerm)) {
            return false;
        }
        return true;
    });
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-card rounded-xl border border-border p-4">
                    ...
                    <div className="text-2xl font-bold text-red-600">{stats.totalAmount.toLocaleString('ar-EG')}</div>
                </div>
            </div>
                            ...
                            {filteredReturns.map((ret) => (
                                <tr key={ret.id} className="border-b border-border/50 hover:bg-muted/30">
                                    <td className="py-3 px-4">
                                        <Link href={`/dashboard/sales/returns/${ret.id}`} className="font-medium hover:text-primary">
                                            {ret.return_number}
                                        </Link>
                                    </td>
                                    ...
                                    <td className="py-3 px-4 text-sm font-mono text-primary hover:underline cursor-pointer">
                                        {ret.original_invoice || '-'}
                                    </td>
                                    <td className="py-3 px-4 text-sm">{ret.customer_name || '-'}</td>
                                    <td className="py-3 px-4 text-sm text-muted-foreground">{ret.reason || '-'}</td>
                                    <td className="py-3 px-4 text-left font-mono font-semibold text-red-600">
                                    ...
                                    <td className="py-3 px-4 text-center">
                                        <Link
                                            href={`/dashboard/sales/returns/${ret.id}`}
