...
export async function GET(request: NextRequest) {
    try {
        ...
        // Get purchase invoices - RLS filters by company_id
        let query = supabase
            .from('purchase_invoices')
            .select('id, invoice_number, invoice_date, total, paid_amount, status, supplier:suppliers(name, name_ar)')
        ...
        const summary = {
            ...
            totalPurchases: invoices?.reduce((sum, inv) => sum + (inv.total || 0), 0) || 0,
