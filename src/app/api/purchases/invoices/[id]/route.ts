...
export async function GET(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        const { data: invoice, error } = await supabase
            ...
            .select(`
                *,
                supplier:suppliers(id, code, name, name_ar, phone, email, address, balance),
                branch:branches(id, code, name, name_ar),
                warehouse:warehouses(id, code, name, name_ar),
                created_by_user:app_users!purchase_invoices_created_by_fkey(id, name, email),
...
// DELETE: Delete purchase invoice (only drafts)
