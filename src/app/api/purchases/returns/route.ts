...
export async function GET(request: NextRequest) {
    try {
        ...
        let query = supabase
            ...
            .select(`
                *,
                ...
                original_invoice:purchase_invoices(id, invoice_number),
                items:purchase_return_items(count)
            `, { count: 'exact' });
        ...
        console.error('Purchase returns error:', error);
...
export async function POST(request: NextRequest) {
    try {
        ...
        let returnNumber = 'PR-00001';
        if (seqData) {
            returnNumber = `${seqData.prefix || 'PR-'}${String(seqData.next_number).padStart(seqData.padding || 5, '0')}`;
            await supabase
                .from('number_sequences')
